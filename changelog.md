# Changelog

#### 3.0.0
* Migrated Dokka configuration to the Dokka Gradle plugin v2 DSL and enabled Dokka v2 mode to remove deprecated v1 task usage.
* Added a backward-compatible `dokkaHtml` alias task so existing invocations continue to work with Dokka v2 tasks.
* Updated CI to build Dokka during pull requests and updated the docs publish workflow output path to `build/dokka/html`.
* Updated Kotlin compiler API version targeting to Kotlin 2.3.
* Updated dependency and plugin versions using `./gradlew refreshVersions` (including Bucket4j 8.16.1, Kotlin 2.3.10, Kotest 6.1.3, and coroutines 1.10.2).
* Updated Kotlin compiler API version targeting to Kotlin 2.3.
* Bumped package version from `2.0.0` to `3.0.0`.

#### 2.0.0
* Updated to build for JDK17 ahead of the OpenJDK 11 EOL date.
* Updated dependencies

#### 1.1.0
Updated Bucket4J to version 8.9.0 and added support for the new [Bandwidth builder API](https://bucket4j.com/8.9.0/toc.html#bandwidth).

#### 1.0.0
Initial release.
